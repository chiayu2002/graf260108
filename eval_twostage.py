"""
Evaluate a trained Two-Stage GRAF model.
...
Usage:
  # Evaluate NeRF only (Phase 1 checkpoint)
  python eval_twostage.py --config ... --checkpoint ... --all

  # Evaluate NeRF + SR (Phase 2 checkpoint)
  python eval_twostage.py --config ... --checkpoint ... --all --sr

  # Quick check
  python eval_twostage.py --config ... --checkpoint ... --quick --sr
"""
import argparse
import csv
import json
...
# =============================================================================
# SR Network (must match train_twostage.py definition)
# =============================================================================
class _RB(nn.Module):
    def __init__(self, c):
        super().__init__()
...
def aggregate(values, name):
    ...
    return {
        f'{name}_mean': float(arr.mean()), f'{name}_std': float(arr.std()),
        f'{name}_min': float(arr.min()), f'{name}_max': float(arr.max()),
...
def eval_paired_metrics(generator, evaluator, sr_network, loader, zdist, v_list,
    ...
    """
    Paired metrics (PSNR / SSIM / MSE / R²).

    如果 use_sr=True 且 sr_network 不是 None:
      - NeRF render 出的圖先 resize 到 64×64，過 SR 放大到 256×256
      - 跟原始 256×256 real image 比較
    否則:
      - NeRF render 出 evaluator 預設大小的圖
      - 跟 real image（resize 到同大小）比較
    """
    generator.eval()
    if sr_network is not None:
        sr_network.eval()

    # 分別收集 nerf_only 和 sr 的 metrics
    ...
    for batch in loader:
        ...
        # NeRF output size (from evaluator, typically 64×64 or 256×256)
        nerf_h, nerf_w = rgb_nerf.shape[2], rgb_nerf.shape[3]

        # NeRF vs real (resize real to match nerf size)
        ...
        # SR vs real (if enabled)
        if use_sr and sr_network is not None:
            with torch.no_grad():
                # 確保輸入資料在 GPU 上 (與 sr_network 同一裝置)
            ...
            real_sr_01 = to_01(real_imgs).cpu()  # 原始 256×256
    ...
    def matched_sample_gen():
        while True:
            ...
            for idx in indices:
                ...
                labels_list.append(label_i); hs_list.append(hs_i)
            ...
            with torch.no_grad():
                ...
                # 強制確保 rgb 在 GPU 上，因為 evaluator 可能回傳 CPU tensor
                rgb = rgb.to(device) 

                # 如果啟用 SR，把 NeRF 輸出過 SR
                if use_sr and sr_network is not None:
                    nerf_h = rgb.shape[2]
                    if nerf_h != 64:
                        rgb_for_sr = F.interpolate(rgb, size=(64,64),
                                                   mode='bilinear', align_corners=True)
                    else:
                        rgb_for_sr = rgb
                    
                    # 現在 rgb_for_sr 確定在 GPU 上了
                    rgb = sr_network(rgb_for_sr)
...
def save_viewpoint_grid(generator, evaluator, sr_network, zdist, cached_hs,
    ...
    """Save multi-specimen viewpoint grid — NeRF + SR side by side."""
    N_views = 8
    ...
    for spec_name, (vec, mat_feat) in specimens.items():
        if spec_name not in cached_hs:
            continue

        labels_list = [vec + mat_feat + [float(a)] for a in angles]
    ...
    if rgb_panels_nerf:
        ...
        out_path = os.path.join(eval_dir, f'grid_nerf_it{it}.png')
        save_image(grid, out_path, nrow=N_views)
        print(f"  Saved NeRF grid ({' / '.join(spec_names)}) -> {out_path}")

        # Per-specimen NeRF
        for i, sn in enumerate(spec_names):
            sp = to_01(rgb_panels_nerf[i])
            save_image(sp, os.path.join(eval_dir, f'nerf_{sn}_it{it}.png'), nrow=N_views)

    if rgb_panels_sr:
        # SR grid
        sr_all = torch.cat(rgb_panels_sr, dim=0)
        grid_sr = to_01(sr_all)
        grid_sr = (grid_sr * 255).to(torch.uint8).float() / 255
        out_path_sr = os.path.join(eval_dir, f'grid_sr256_it{it}.png')
        save_image(grid_sr, out_path_sr, nrow=N_views)
        print(f"  Saved SR 256 grid -> {out_path_sr}")

        # Per-specimen SR
        for i, sn in enumerate(spec_names):
            sp_sr = to_01(rgb_panels_sr[i])
            save_image(sp_sr, os.path.join(eval_dir, f'sr256_{sn}_it{it}.png'), nrow=N_views)
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description='Evaluate Two-Stage GRAF model.')
    parser.add_argument('--config', type=str, required=True)
    parser.add_argument('--checkpoint', type=str, required=True)
    parser.add_argument('--num_pairs', type=int, default=100)
    parser.add_argument('--sr', action='store_true',
                        help='Also evaluate SR 256×256 output (requires SR in checkpoint)')
    ...
    extractor_args = json.load(open(
        "HystereticGRU/2026-03-24_17-13-01/args.json", "r"))
    ...
    # SR network (建立但可能 checkpoint 裡沒有)
    ...
    # 檢查 SR 是否真的有載入權重
    sr_loaded = False
    if sr_network is not None and 'sr_network' in load_dict:
        sr_loaded = True
        print(f"[SR] Weights loaded from checkpoint")
    elif sr_network is not None:
        # checkpoint_io.parse_state_dict 會印 warning
        # 如果 checkpoint 裡沒有 sr_network，權重是隨機的
        print(f"[SR] WARNING: sr_network not found in checkpoint, using random weights!")
        print(f"[SR] SR eval results will be meaningless. Use --sr only with Phase 2 checkpoints.")

    # Sanity check
    nerf_model = generator.render_kwargs_train['network_fn']
    print(f"[Sanity] condition_feature norm = {nerf_model.condition_feature.weight.norm():.4f}")
    ...
    # 1. Paired metrics
    # =================================================================
    metrics_nerf = {}; per_sample_nerf = {'psnr':[],'ssim':[],'mse':[],'r2':[]}
    per_spec_nerf = {}
    metrics_sr = {}; per_sample_sr = {'psnr':[],'ssim':[],'mse':[],'r2':[]}
    per_spec_sr = {}
    ...
    if args.psnr_r2:
        ...
        print("  " + "-"*32)
        for name in ['psnr','ssim','mse','r2']:
            print(f"  {name.upper():<12} {metrics_nerf.get(f'{name}_mean',0):>10.4f} "
        ...
        if metrics_sr:
            ...
            print("  " + "-"*32)
            for name in ['psnr','ssim','mse','r2']:
                print(f"  {name.upper():<12} {metrics_sr.get(f'sr_{name}_mean',0):>10.4f} "
    ...
    if args.fid_kid:
        ...
        print(f"    NeRF FID={fid_nerf:.2f}, KID×100={kid_nerf*100:.2f} ({time.time()-t0:.1f}s)")
        ...
        if args.sr and sr_network is not None:
            ...
            print(f"    SR FID={fid_sr:.2f}, KID×100={kid_sr*100:.2f} ({time.time()-t0:.1f}s)")
            torch.cuda.empty_cache()
    ...
    write_csv(csv_path, metrics_nerf, per_sample_nerf, per_spec_nerf,
              metrics_sr, per_sample_sr, per_spec_sr,
